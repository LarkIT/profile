...
class profile::solr::client (
  ...
  if str2bool($::settings::storeconfigs) {
    ...
    @@firewall { "200 INPUT allow solr access from ${::ipaddress} to ${port}":
      ...
      chain  => 'INPUT',
      source => $::ipaddress,
      tag    => "fw_${::environment}_${::client}_${::app_tier}_${::app_name}_solr_in_from_clients",
    }
    # Allow inbound connections.
    Firewall <<| tag == "fw_${::environment}_${::client}_${::app_tier}_${::app_name}_solr_out_to_solr_server" |>>

    # Pick up the hosts file entry that was left for us.
    Host <<| tag == "host_${::environment}_${::client}_${::app_tier}_${::app_name}_solr_server" |>>
  }
